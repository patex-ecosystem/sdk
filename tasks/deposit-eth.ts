...
task('deposit-eth', 'Deposits ether to L2.')
  ...
  .setAction(async (args, hre) => {
    ...
    if (args.l1ContractsJsonPath) {
      ...
      const Deployment__PatexPortal = await hre.deployments.get(
        'PatexPortalProxy'
      )
      ...
      contractAddrs = {
        l1: {
          ...
          PatexPortal: Deployment__PatexPortal.address,
    ...
    console.log(
      `PatexPortal balance before: ${formatEther(opBalanceBefore)}`
    )
    console.log(`PatexPortal balance after: ${formatEther(opBalanceAfter)}`)

    if (!opBalanceBefore.add(amount).eq(opBalanceAfter)) {
      throw new Error(`PatexPortal balance mismatch`)
    }
