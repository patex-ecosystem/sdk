...
} from '@eth-patex/contracts-bedrock'
import { sleep } from '@eth-patex/core-utils'
...
const creatPatexPatexMintableERC20 = async (
  hre: HardhatRuntimeEnvironment,
  L1ERC20: Contract,
  l2Signer: Wallet
): Promise<Contract> => {
  const Artifact__PatexMintableERC20Token = await getContractDefinition(
    'PatexMintableERC20'
  )

  const Artifact__PatexMintableERC20TokenFactory =
    await getContractDefinition('PatexMintableERC20Factory')

  const PatexMintableERC20TokenFactory = new Contract(
    predeploys.PatexMintableERC20Factory,
    Artifact__PatexMintableERC20TokenFactory.abi,
    l2Signer
  )
  ...
  const tx =
    await PatexMintableERC20TokenFactory.createPatexMintableERC20(
      L1ERC20.address,
      `L2 ${name}`,
      `L2-${symbol}`
    )
  ...
  const event = receipt.events.find(
    (e: Event) => e.event === 'PatexMintableERC20Created'
  )

  if (!event) {
    throw new Error('Unable to find PatexMintableERC20Created event')
  }
  ...
  return new Contract(
    l2WethAddress,
    Artifact__PatexMintableERC20Token.abi,
    l2Signer
  )
}
...
task('deposit-erc20', 'Deposits WETH9 onto L2.')
  ...
  .setAction(async (args, hre) => {
    ...
    const Artifact__PatexPortal = await getContractDefinition(
      'PatexPortal'
    )
    ...
    const PatexPortal = new hre.ethers.Contract(
      contractAddrs.l1.PatexPortal,
      Artifact__PatexPortal.abi,
      signer
    )
    ...
    const PatexMintableERC20 = await createPatexMintableERC20(
      hre,
      WETH9,
      l2Signer
    )
    ...
    const l2Balance = await PatexMintableERC20.balanceOf(address)
    ...
    for (const log of finalizeReceipt.logs) {
      switch (log.address) {
        case PatexPortal.address: {
          const parsed = PatexPortal.interface.parseLog(log)
          console.log(`Log ${parsed.name} from PatexPortal (${log.address})`)
          console.log(parsed.args)
          console.log()
          break
        }
