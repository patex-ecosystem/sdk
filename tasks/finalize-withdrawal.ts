...
import { predeploys } from '@eth-patex/contracts-bedrock'
...
task('finalize-withdrawal', 'Finalize a withdrawal')
  ...
  .setAction(async (args, hre: HardhatRuntimeEnvironment) => {
    ...
    const Deployment__PatexPortalProxy = await hre.deployments.getOrNull(
      'PatexPortalProxy'
    )
    ...
    const messenger = new CrossChainMessenger({
      ...
      contracts: {
        l1: {
          ...
          PatexPortal: Deployment__PatexPortalProxy?.address,
        },
      },
    })
