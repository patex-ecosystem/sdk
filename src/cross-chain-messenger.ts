...
} from '@eth-patex/core-utils'
import { getContractInterface, predeploys } from '@eth-patex/contracts'
...
export class CrossChainMessenger {
  ...
  //     getMessagesByAddress will be enabled by plugging in an Patex Indexer (coming soon).
  //   `)
  // }
      ...
      if (receipt === null) {
        ...
        if (this.bedrock) {
          ...
          const provenWithdrawal =
            await this.contracts.l1.PatexPortal.provenWithdrawals(
    ...
    const challengePeriod =
      oracleVersion === '1.0.0'
        ? // The ABI in the SDK does not contain FINALIZATION_PERIOD_SECONDS
          // in PatexPortal, so making an explicit call instead.
          BigNumber.from(
            await this.contracts.l1.PatexPortal.provider.call({
              to: this.contracts.l1.PatexPortal.address,
  ...
  /**
   * Queries the PatexPortal contract's `provenWithdrawals` mapping
  ...
  public async getProvenWithdrawal(
    ...
    return this.contracts.l1.PatexPortal.provenWithdrawals(withdrawalHash)
  }
  ...
  public async getMessageProof(
    ...
    // https://github.com/patex-ecosystem/contracts/contracts/L2/predeploys/OVM_L2ToL1MessagePasser.sol#L23
  ...
  populateTransaction = {
    ...
    proveMessage: async (
      ...
      return this.contracts.l1.PatexPortal.populateTransaction.proveWithdrawalTransaction(
        [
    ...
    finalizeMessage: async (
      ...
      if (this.bedrock) {
        const withdrawal = await this.toLowLevelMessage(resolved)
        return this.contracts.l1.PatexPortal.populateTransaction.finalizeWithdrawalTransaction(
          [
            withdrawal.messageNonce,
            withdrawal.sender,
            withdrawal.target,
